...
export default function ViewFeedbackPage() {
	...
	return (
		<main className="flex flex-col md:flex-row gap-8 p-8 rounded-xl">
			{/* Document Viewer */}
				...
				{submissionFile ? (
					<div className="h-[80svh] bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
						{submissionFile?.type === "application/pdf" ? (
							<PDFObject containerProps={{ className: "h-full" }} url={URL.createObjectURL(submissionFile)} height="100%" width="100%" />
						) : (
							<div className="p-8 flex flex-col items-center justify-center h-full">
								...
								<p className="text-sm text-gray-500 text-center">For non-PDF files, this is just a preview placeholder.</p>
							</div>
						)}
					</div>
				) : (
			...
			<aside className="flex flex-col flex-2 gap-4 col-span-2">
				...
				<div className="rounded-xl overflow-hidden shadow-md bg-white">
					...
					<div className="flex flex-col gap-4 p-4 bg-white">
						...
						{similarity ? <p> {similarity?.summary} </p> : <Skeleton className="h-[1ch] w-[10ch] bg-black/30" />}
				...
				<section>
					<div
						className={
							"w-full bg-yellow-300 px-4 py-1.5 font-bold flex justify-between items-center cursor-pointer" +
							(expandStella ? " rounded-t-lg " : " rounded-lg ")
						}
						onClick={() => {
							if (stellaFeedback) setExpandStella(!expandStella);
						}}
					>
					...
					<div className={`bg-yellow-100 border border-yellow-300 rounded-b-lg p-4 ${expandStella ? "visible" : "hidden"}`}>
						{stellaFeedback ? (
							<div className="flex flex-col gap-2">
								<p className="flex-1">{stellaFeedback.general}</p>
								...
								{expandStella && <div className="animate-fadeIn"></div>}
							</div>
						) : (
				...
				<section>
					<div
						className={`w-full bg-gray-300 rounded-t-lg px-4 py-1.5 flex justify-between items-center cursor-pointer ${
							expandInstructor ? " rounded-t-lg " : " rounded-lg "
						}`}
						onClick={() => {
							if (instructorFeedback) setExpandInstructor(!expandInstructor);
						}}
					>
						<span className="text-gray-800 font-bold"> INSTRUCTORS&apos; COMMENTS </span>
						<span>{expandInstructor ? "▼" : "►"}</span>
					</div>
					<div
						className={`border bg-gray-100  border-gray-300 rounded-b-lg p-4 transition-all duration-300 overflow-hidden ${
							expandInstructor ? "visible" : "hidden"
						}`}
					>
						<div className="flex">
							<div className="flex-1">{instructorFeedback ?? <Skeleton className="h-[1ch] w-[10ch] bg-black/30" />}</div>
						</div>
					</div>
				</section>
			</aside>
