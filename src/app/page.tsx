    ...
    <div className="md:max-h-[10vh]">
      <nav className="ml-4 w-fit flex flex-row gap-0  bg-gray-100 "> 
        <div
          ...
          <div
            key={'feedback'}
            className={`text-gray-300 rounded-t-2xl border-2 px-8 py-2 cursor-pointer ${currentTab === 'feedback' ? selectedState : ''} ${enableFeedbackTab ? 'text-gray-800' : 'bg-gray-400 '}`}
            onClick={() => { if (enableFeedbackTab) setCurrentTab('feedback')}}>
            View Feedback
          </div>
      </nav>
